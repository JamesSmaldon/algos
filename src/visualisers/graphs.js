...
function create_vis_graph(graph) {
    ...
    function add_vis_data(node) {
        var val = node.value;
        node.value = { pos: new M.Vector(0,0), 
                       prev_pos: new M.Vector(0,0),
                       force: new M.Vector(0,0),
                       value: val };
    }
...
function set_initial_pos(graph) {
    graph.nodes[0].value.pos = new M.Vector(300, 200);
    graph.nodes[1].value.pos = new M.Vector(400, 300);
    graph.nodes[2].value.pos = new M.Vector(300, 400);
    graph.nodes[3].value.pos = new M.Vector(200, 300);
    graph.nodes[4].value.pos = new M.Vector(0, 0);
    graph.nodes[0].value.prev_pos = new M.Vector(300, 200);
    graph.nodes[1].value.prev_pos = new M.Vector(400, 300);
    graph.nodes[2].value.prev_pos = new M.Vector(300, 400);
    graph.nodes[3].value.prev_pos = new M.Vector(200, 300);
    graph.nodes[4].value.prev_pos = new M.Vector(0, 0);
}
...
function create_graph_vis(graph) {
    ...
    g = create_vis_graph(DU.create_cycle(5));
    set_initial_pos(g);
...
function update() {
    for (var i=0; i<g.nodes.length; ++i) {
        g.nodes[i].value.force = new M.Vector(0,0);
    }
